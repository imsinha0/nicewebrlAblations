...
class PlaygroundTimestepWrapper(TimestepWrapper):
  ...
  def step(self, key, state, action, params=None):
    inner_state = state.state.replace(grid=jnp.asarray(state.state.grid))
    new_state_for_env_step = state.replace(state=inner_state)
    timestep = self._env.step(
      params=params, timestep=new_state_for_env_step, action=action
    )
    resized_obs = jax.image.resize(
      timestep.observation, shape=(256, 256, 3), method="bilinear"
    ).astype(jnp.uint8)
